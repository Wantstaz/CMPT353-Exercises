...
def main():
    ...
    # Gaussian Naive Bayes (RGB)
    bayes_rgb_model = GaussianNB()
    bayes_rgb_model.fit(X_train, y_train)
    
    # Gaussian Naive Bayes (Convert)
    bayes_convert_model = make_pipeline(
        FunctionTransformer(rgb2lab, validate=False),
        GaussianNB()
    )
    ...
    # Random Forest (RGB)
    rf_rgb_model = RandomForestClassifier(n_estimators=500, max_depth=20, min_samples_leaf=5)
    rf_rgb_model.fit(X_train, y_train)

    # Random Forest (Convert)
    rf_convert_model = make_pipeline(
        FunctionTransformer(rgb2lab),
        RandomForestClassifier(n_estimators=500, max_depth=20, min_samples_leaf=5)
    )
